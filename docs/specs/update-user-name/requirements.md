...
本要件のバリデーションは以下の順序で実行される:

1. **正規化（Normalization）:** 入力値の前後空白を除去して正規値を得る
2. **バリデーション（Validation）:** 正規値に対して必須・長さの制約を評価する

#### Acceptance Criteria

1. WHEN 表示名に前後空白が含まれる THEN システム SHALL 前後空白を除去した正規値で保存する
2. WHEN 表示名が以下のいずれかに該当する THEN システム SHALL 入力不備として拒否する:
   - 表示名がリクエストに含まれていない
   - 値が未設定である
   - 値が空である
   - 正規化後に空となる（空白のみの入力等）
3. WHEN 正規化後の表示名が 50 文字を超える THEN システム SHALL 入力不備として拒否する
